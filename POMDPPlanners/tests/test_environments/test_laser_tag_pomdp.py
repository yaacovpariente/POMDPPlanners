...
class TestLaserTagPOMDP:
    ...
    def test_compute_metrics_with_simulator_generated_history(self):
        ...
        for metric in metrics:
            assert isinstance(metric.value, (int, float)), f"Metric {metric.name} value must be numeric"
            assert metric.value >= 0, f"Metric {metric.name} value must be non-negative: {metric.value}"
            assert hasattr(metric, 'lower_confidence_bound'), f"Metric {metric.name} missing lower confidence bound"
            assert hasattr(metric, 'upper_confidence_bound'), f"Metric {metric.name} missing upper confidence bound"
            
            # Validate specific metric ranges
            if metric.name == 'tag_success_rate':
                assert 0.0 <= metric.value <= 1.0, f"Tag success rate must be between 0 and 1: {metric.value}"
            elif metric.name == 'average_episode_length':
                assert metric.value <= 15, f"Average episode length should not exceed max: {metric.value}"
        ...
        for history in histories:
            ...
            for step in history.history:
                ...
                assert step.action in env.get_actions(), f"Action {step.action} not in valid actions"
                ...
                assert 0 <= robot_pos[0] < env.floor_shape[0], f"Robot row {robot_pos[0]} out of bounds"
                assert 0 <= robot_pos[1] < env.floor_shape[1], f"Robot col {robot_pos[1]} out of bounds"
                assert 0 <= opponent_pos[0] < env.floor_shape[0], f"Opponent row {opponent_pos[0]} out of bounds"
                assert 0 <= opponent_pos[1] < env.floor_shape[1], f"Opponent col {opponent_pos[1]} out of bounds"
    ...
    def test_compute_metrics_handles_none_rewards(self):
        ...
        try:
            ...
            assert success_rate_metric.value == 1.0, f"Expected success rate 1.0, got {success_rate_metric.value}"
            ...
            assert failed_tags_metric.value == 0.0, f"Expected 0.0 failed tags, got {failed_tags_metric.value}"
