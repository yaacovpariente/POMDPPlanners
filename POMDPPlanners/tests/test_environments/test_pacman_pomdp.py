...
class TestPacManState:
    ...
    def test_pacman_state_validation_invalid_pellets(self):
        ...
        Test type: unit
        """
        with pytest.raises(ValueError, match="pellets must be a tuple of position tuples"):
            PacManState(
                pacman_pos=(1, 2),
                ghost_positions=((3, 4),),
...
class TestMultiGhostFeatures:
    ...
    def test_multi_ghost_collision_detection(self):
        ...
        assert any(
            ghost_pos == state_collision.pacman_pos for ghost_pos in state_collision.ghost_positions
        )
    ...
    def test_multi_ghost_state_transitions(self):
        ...
        assert all(isinstance(pos, tuple) and len(pos) == 2 for pos in next_state.ghost_positions)
...
class TestSampleNextStepEquivalence:
    ...
    def test_sample_next_step_matches_base_class(self):
        ...
        Given: A PacManPOMDP environment and valid (state, action) pairs
        ...
        for action in range(5):
            for _ in range(20):
                ...
                assert (
                    opt_next == base_next
                ), f"next_state mismatch for action={action}, seed={seed}"
                assert opt_obs == base_obs, f"observation mismatch for action={action}, seed={seed}"
