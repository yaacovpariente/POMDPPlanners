"""Tests for the CartPole Gaussian belief factory.

This module tests the factory function and enum that produce pre-configured
GaussianBelief instances for the CartPolePOMDP environment.
"""
...
class TestFactoryDispatch:
    def test_ekf_returns_ekf_updater(self, env, initial_cov):
        ...
        Given: A CartPolePOMDP and EKF updater type.
        When: create_cartpole_gaussian_belief is called.
    ...
    def test_ukf_returns_ukf_updater(self, env, initial_cov):
        ...
        Given: A CartPolePOMDP and UKF updater type.
        When: create_cartpole_gaussian_belief is called.
...
class TestEKFFunctions:
    ...
    def test_ekf_observation_jacobian_is_identity(self, env, initial_cov):
        """Test that the EKF observation Jacobian is I_4.
        ...
        updater = belief.updater
        assert isinstance(updater, ExtendedKalmanFilterUpdater)
        H = updater.observation_jacobian(np.zeros(4))
        np.testing.assert_array_equal(H, np.eye(4))
    ...
    def test_ekf_transition_jacobian_near_numerical(self, env, initial_cov):
        ...
        state = np.array([0.01, -0.02, 0.05, 0.03])
        action = np.array([1.0])
        eps = 1e-6

        analytical_J = updater.transition_jacobian(state, action)

        numerical_J = np.zeros((4, 4))
        for i in range(4):
...
class TestBeliefAttributes:
    def test_mean_is_zero(self, env, initial_cov):
        """Test that the returned belief mean is zero (center of initial dist).

        Purpose: Validates mean initialisation to center of Uniform(-0.05, 0.05).

        Given: A CartPolePOMDP environment.
        When: Factory creates a belief.
        Then: belief.mean == np.zeros(4).
        ...
        np.testing.assert_array_equal(belief.mean, np.zeros(4))
    ...
    def test_covariance_matches_initial(self, env, initial_cov):
        ...
        Given: initial_covariance = 0.01 * I_4.
        When: Factory creates a belief.
    ...
    def test_default_covariance(self, env):
        """Test that the default covariance matches Uniform(-0.05, 0.05) variance.
        ...
        Then: belief.covariance == np.eye(4) * (0.1**2 / 12).
        ...
        expected = np.eye(4) * (0.1**2 / 12.0)
        np.testing.assert_allclose(belief.covariance, expected)

    def test_mean_shape_is_4d(self, env, initial_cov):
        """Test that belief mean has shape (4,).

        Purpose: Validates output dimensionality.

        Given: A CartPolePOMDP (4D state).
        When: Factory creates a belief.
        Then: belief.mean.shape == (4,) and belief.dim == 4.
...
class TestNoiseParameters:
    def test_ekf_uses_env_noise_cov_as_R(self, env, initial_cov):
        """Test that the EKF updater R matches env.noise_cov.

        Purpose: Validates that observation noise is taken from environment.

        Given: A CartPolePOMDP with known noise_cov.
        When: EKF belief is created.
        Then: updater.R == env.noise_cov.
    ...
    def test_process_noise_scale_controls_Q(self, env, initial_cov):
        ...
        updater = belief.updater
        assert isinstance(updater, ExtendedKalmanFilterUpdater)
        np.testing.assert_array_equal(updater.Q, np.eye(4) * scale)
...
class TestEndToEndUpdate:
    ...
    def test_update_produces_valid_gaussian_belief(self, env, initial_cov, updater_type):
        ...
        Given: A CartPole GaussianBelief with the given updater type.
        ...
        assert new_belief.mean.shape == (4,)
        assert new_belief.covariance.shape == (4, 4)
    ...
    def test_ekf_ukf_produce_similar_results(self, env):
        ...
        """
        cov = np.eye(4) * 0.01
        belief_ekf = create_cartpole_gaussian_belief(
            env=env,
            updater_type=GaussianBeliefUpdaterType.EKF,
            initial_covariance=cov,
        )
        belief_ukf = create_cartpole_gaussian_belief(
            env=env,
            updater_type=GaussianBeliefUpdaterType.UKF,
            initial_covariance=cov,
        )
    ...
    def test_multi_step_update(self, env):
        ...
        Given: A CartPole GaussianBelief with EKF updater.
        ...
        for _ in range(5):
            action_val = np.random.choice([0, 1])
            next_state, observation, _ = env.sample_next_step(state, action_val)
...
class TestSampling:
    def test_sample_returns_correct_shape(self, env, initial_cov):
        """Test that sampling from the Gaussian belief returns shape (4,).

        Purpose: Validates sample output dimensionality.

        Given: A CartPole GaussianBelief.
        When: belief.sample() is called.
        Then: The returned state has shape (4,).
...
class TestBeliefFactoryIntegration:
    def test_create_cartpole_belief_gaussian_type(self, env):
        """Test that create_cartpole_belief dispatches to Gaussian factory.
        ...
        assert isinstance(belief, GaussianBelief)
        assert belief.mean.shape == (4,)
        assert belief.covariance.shape == (4, 4)

    def test_create_cartpole_belief_gaussian_with_kwargs(self, env):
        """Test that create_cartpole_belief forwards kwargs to Gaussian factory.

        Purpose: Validates kwargs forwarding for Gaussian belief creation.

        Given: A CartPolePOMDP environment.
        When: create_cartpole_belief is called with GAUSSIAN and custom kwargs.
