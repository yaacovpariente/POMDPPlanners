...
def test_scene_geometry_and_background_unchanged(visualizer):
    ...
    assert background.getpixel(tuple(map(round, visualizer._world_to_pixel((5.0, 3.0))))) == (
        76,
        76,
        76,
    )
    danger = background.getpixel(tuple(map(round, visualizer._world_to_pixel((3.2, 5.2)))))
    assert danger[0] > danger[1] == danger[2]
...
def test_laser_stops_at_wall_and_beliefs_clear(visualizer):
    ...
    assert frame.getpixel((x, y)) == (76, 76, 76)
    step = history()[0]
...
def test_tag_and_action_semantics(visualizer):
    ...
    bounds = (
        int(visualizer._left),
        int(visualizer._bottom - 55),
        int(visualizer._left + 220),
        int(visualizer._bottom),
    )
...
def test_danger_fields_are_clipped_to_world_margin(visualizer):
    ...
    assert background.getpixel((x, y)) == (255, 255, 255)
