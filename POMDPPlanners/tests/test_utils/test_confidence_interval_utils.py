...
def test_verify_metrics_valid_intervals():
    ...
    """
    metrics = [
        MetricValue(name="test_metric", value=5.0, lower_confidence_bound=4.0, upper_confidence_bound=6.0),
        MetricValue(name="zero_metric", value=0.0, lower_confidence_bound=0.0, upper_confidence_bound=0.0),
        MetricValue(name="narrow_metric", value=10.0, lower_confidence_bound=10.0, upper_confidence_bound=10.0)
    ]
...
def test_verify_metrics_outside_intervals():
    ...
    """
    metrics = [
        MetricValue(name="outside_metric", value=5.0, lower_confidence_bound=6.0, upper_confidence_bound=8.0)
    ]
    
    with pytest.raises(AssertionError, match="outside_metric value 5.0 not within confidence interval"):
        verify_metrics_within_confidence_intervals(metrics)
...
def test_verify_metrics_invalid_bounds():
    ...
    """
    metrics = [
        MetricValue(name="invalid_bounds", value=5.0, lower_confidence_bound=8.0, upper_confidence_bound=6.0)
    ]
...
def test_verify_metrics_with_tolerance():
    ...
    """
    metrics = [
        MetricValue(name="tolerance_metric", value=5.1, lower_confidence_bound=4.0, upper_confidence_bound=5.0)
    ]
...
def test_verify_metrics_with_wide_intervals():
    ...
    """
    metrics = [
        MetricValue(name="wide_metric", value=1.0, lower_confidence_bound=0.0, upper_confidence_bound=5.0)  # 500% relative width
    ]
