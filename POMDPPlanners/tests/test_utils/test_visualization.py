...
def test_plot_tree_graphs(temp_cache_dir):
    ...
    try:
        ...
        assert "display" in str(e).lower() or "show" in str(e).lower() or "browser" in str(e).lower()
...
def test_plot_discounted_returns_histogram_multiple_policies_empty_histories(temp_cache_dir):
...
def test_plot_discounted_returns_histogram_multiple_policies_single_history_per_policy(temp_cache_dir):
...
def test_plot_tree_graphs_deep_tree(temp_cache_dir):
    ...
    for i in range(5):  # Create 5 levels deep
        ...
        belief_node = BeliefNode(
            belief=WeightedParticleBelief(
                particles=[np.array([float(i), float(i)])],
                log_weights=np.array([0.1])
            ),
            observation=f"obs_{i}",
            parent=action_node
