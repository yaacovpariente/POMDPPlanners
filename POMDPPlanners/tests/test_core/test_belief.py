...
def test_is_terminal_belief_large_particle_set():
    ...
    particles = []
    for i in range(100):
        terminal_state = LaserTagState(robot=(i % 7, i % 11), opponent=((i+1) % 7, (i+1) % 11), terminal=True)
...
def test_is_terminal_belief_large_mixed_particle_set():
    ...
    for i in range(100):
        ...
        state = LaserTagState(robot=(i % 7, i % 11), opponent=((i+1) % 7, (i+1) % 11), terminal=terminal)
...
def test_is_terminal_belief_comprehensive_scenarios():
    ...
    log_weights = np.log(np.ones(3) / 3)
    belief_all_terminal = WeightedParticleBelief(particles=all_terminal_particles, log_weights=log_weights)
    ...
    belief_all_non_terminal = WeightedParticleBelief(particles=all_non_terminal_particles, log_weights=log_weights)
    ...
    belief_mixed_majority_terminal = WeightedParticleBelief(particles=mixed_particles_majority_terminal, log_weights=log_weights)
    ...
    belief_mixed_majority_non_terminal = WeightedParticleBelief(particles=mixed_particles_majority_non_terminal, log_weights=log_weights)
