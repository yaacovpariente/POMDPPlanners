...
class TestPathSimulationPolicyInitialization:
    ...
    def test_continuous_environment_initialization_with_action_sampler(self, continuous_environment, action_sampler):
    ...
    def test_both_n_simulations_and_timeout_raises_error(self, discrete_environment):
        ...
        Test type: unit
        """
        with pytest.raises(ValueError, match="Cannot specify both n_simulations and time_out_in_seconds"):
    ...
    def test_continuous_environment_without_action_sampler_raises_error(self, continuous_environment):
        ...
        Test type: unit
        """
        with pytest.raises(ValueError, match="Action sampler must be provided for continuous action spaces"):
            ConcretePathSimulationPolicy(
                environment=continuous_environment,
                discount_factor=0.95,
                name="test_policy",
    ...
    def test_neither_n_simulations_nor_timeout_in_learn_tree_raises_error(self, discrete_environment):
        ...
        with pytest.raises(ValueError, match="Either n_simulations or time_out_in_seconds must be provided"):
...
class TestPathSimulationPolicyTerminalBeliefHandling:
    ...
    def test_terminal_belief_returns_random_action_discrete(self, mock_is_terminal, discrete_environment):
    ...
    @patch('POMDPPlanners.planners.mcts_planners.path_simulations_policy.is_terminal_belief')
    def test_terminal_belief_returns_random_action_continuous(self, mock_is_terminal, continuous_environment, action_sampler):
        ...
        with patch.object(action_sampler, 'sample', return_value=1.5) as mock_sample:
            ...
            assert isinstance(policy_data, PolicyRunData)
            assert policy_data.info_variables == []
            mock_is_terminal.assert_called_once_with(belief=terminal_belief, env=continuous_environment)
            mock_sample.assert_called_once()

    @patch('POMDPPlanners.planners.mcts_planners.path_simulations_policy.is_terminal_belief')
    @patch('POMDPPlanners.planners.mcts_planners.path_simulations_policy.compute_tree_metrics')
    @patch('POMDPPlanners.planners.mcts_planners.path_simulations_policy.get_optimal_action_reward_setting')
    def test_non_terminal_belief_builds_tree(self, mock_get_optimal, mock_compute_metrics, mock_is_terminal, discrete_environment):
        ...
        mock_compute_metrics.return_value = [PolicyInfoVariable("nodes", 10), PolicyInfoVariable("depth", 3)]
        ...
        assert actions[0] == 1
        assert isinstance(policy_data, PolicyRunData)
        assert policy_data.info_variables == [PolicyInfoVariable("nodes", 10), PolicyInfoVariable("depth", 3)]
...
class TestPathSimulationPolicyAbstractContract:
    ...
    def test_cannot_instantiate_abstract_base_class(self, discrete_environment):
        ...
        Test type: unit
        """
        with pytest.raises(TypeError, match="Can't instantiate abstract class PathSimulationPolicy"):
