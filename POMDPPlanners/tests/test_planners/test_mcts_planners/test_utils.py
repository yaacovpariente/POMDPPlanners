...
def validate_tree_structure_with_progressive_widening(
    ...
    for node in PostOrderIter(root_belief_node):
        ...
        if isinstance(node, BeliefNode):
            ...
            if (
                expected_belief_type is not None and node.depth > 0
            ):  # Non-root nodes created during tree construction
            ...
            assert (
                node.action in action_sampler.get_space()
    ...
    assert (
        belief_node_count >= action_node_count
    ), "Should have at least as many belief nodes as action nodes"
