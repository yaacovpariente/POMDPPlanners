...
def action_progressive_widening(
    ...
    """
    if belief_node.is_leaf or belief_node.visit_count == 0 or floor(belief_node.visit_count ** alpha_a) > floor((belief_node.visit_count - 1) ** alpha_a):
...
def ucb1_exploration(belief_node: BeliefNode, exploration_constant: float) -> ActionNode:
    ...
    children_visit_counts = [max(child.visit_count, 1) for child in belief_node.children]  # Avoid division by zero
    
    ucb = q_vals + exploration_constant * np.sqrt(np.log(belief_node.visit_count) / children_visit_counts)
