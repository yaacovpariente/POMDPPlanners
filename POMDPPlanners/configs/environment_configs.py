...
from POMDPPlanners.utils.weighted_particle_beliefs import WeightedParticleBeliefContinuousLightDarkFullCoverage, WeightedParticleBeliefDiscreteLightDarkFullCoverage

from POMDPPlanners.core.belief import get_initial_belief
from POMDPPlanners.environments.light_dark_pomdp.continuous_light_dark_pomdp import ContinuousLightDarkPOMDP, ContinuousLightDarkPOMDPDiscreteActions, RewardModelType
...
class EnvironmentConfigsAPI:
    ...
    def tiger_pomdp_config(self, n_particles: int = 20) -> Tuple[Environment, WeightedParticleBelief]:
        pomdp = TigerPOMDP(
    ...
    def cartpole_pomdp_config(self, n_particles: int = 20) -> Tuple[Environment, WeightedParticleBelief]:
        # Create noise covariance matrix for CartPole observations
        noise_cov = np.diag([0.1, 0.1, 0.1, 0.1])  # Noise for [cart_pos, cart_vel, pole_angle, pole_vel]
        pomdp = CartPolePOMDP(
            discount_factor=self.discount_factor,
            noise_cov=noise_cov,
            name="CartPolePOMDP"
    ...
    def mountain_car_pomdp_config(self, n_particles: int = 20) -> Tuple[Environment, WeightedParticleBelief]:
    ...
    def continuous_observations_continuous_actions_light_dark_pomdp_config(self, n_particles: int = 20) -> Tuple[Environment, WeightedParticleBelief]:
        DISCOUNT_FACTOR = self.discount_factor
        STATE_TRANSITION_COV_MATRIX = np.eye(2) * 0.075  # Identity matrix for state transitions
        OBSERVATION_COV_MATRIX = np.array([[0.075, 0.01], [0.01, 0.075]])  # Anisotropic observation noise
        BEACONS = [(1, 1), (1, 4), (4, 4), (4, 1)]  # Grid pattern as list of tuples
    ...
    def rock_sample_pomdp_config(self, n_particles: int = 20) -> Tuple[Environment, WeightedParticleBelief]:
    ...
    def laser_tag_pomdp_config(self, n_particles: int = 20) -> Tuple[Environment, WeightedParticleBelief]:
        pomdp = LaserTagPOMDP(
            discount_factor=self.discount_factor,
            name="LaserTagPOMDP"
        )
    ...
    def safety_ant_velocity_pomdp_config(self, n_particles: int = 20) -> Tuple[Environment, WeightedParticleBelief]:
        pomdp = SafeAntVelocityPOMDP(
            discount_factor=self.discount_factor,
            name="SafeAntVelocityPOMDP"
        )
        belief = get_initial_belief(pomdp=pomdp, n_particles=n_particles, resampling=True)
        return pomdp, belief
