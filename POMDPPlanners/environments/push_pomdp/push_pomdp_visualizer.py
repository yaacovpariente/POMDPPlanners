...
class PushPOMDPVisualizer:
    ...
    def create_visualization(self, history: List[StepData], cache_path: Path) -> None:
        ...
        with obstacles, collision detection, distance indicators, belief particles over
        the robot and object positions, and success feedback.
    ...
    def _create_animate_function(
        ...
        def animate(frame: int) -> Tuple[Any, ...]:
            ...
            robot_collision = self.env._is_colliding_with_obstacle(
                robot_pos
            )  # pylint: disable=protected-access
            object_collision = self.env._is_colliding_with_obstacle(
                object_pos
            )  # pylint: disable=protected-access
