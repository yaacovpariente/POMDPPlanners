...
class DiscreteLDObservationModel(ObservationModel):
    def __init__(self, next_state: np.ndarray, action: Any, beacons: np.ndarray, beacon_radius: float, observation_error_prob: float):
...
class DiscreteLightDarkPOMDP(DiscreteActionsEnvironment):
    def __init__(
        ...
        beacons: np.ndarray = np.array([[0, 0, 0, 5, 5, 5, 10, 10, 10], 
                                        [0, 5, 10, 0, 5, 10, 0, 5, 10]]),
        goal_state: np.ndarray = np.array([10, 5]),
        start_state: np.ndarray = np.array([0, 5]),
        obstacles: np.ndarray = np.array([[3, 7], 
                                          [5, 5]]),
        ...
        self.__type_check(
            ...
            beacons=beacons, goal_state=goal_state, 
    ...
    def __type_check(
        ...
        assert isinstance(observation_error_prob, float), "observation_error_prob must be a float"
        ...
        assert isinstance(is_stochastic_reward, bool), "is_stochastic_reward must be a boolean"
        
        # Value range checks
        assert 0 <= discount_factor <= 1, "discount_factor must be between 0 and 1"
        assert 0 <= transition_error_prob <= 1, "transition_error_prob must be between 0 and 1"
        assert 0 <= observation_error_prob <= 1, "observation_error_prob must be between 0 and 1"
    ...
    def visualize_path(self, path: List[np.ndarray], cache_path: Path):
        ...
        # Plot the goal state
        ax.scatter(self.goal_state[0], self.goal_state[1], color='green', label='Goal State')
