...
def create_continuous_laser_tag_belief(
    ...
    Args:
        env: ContinuousLaserTagPOMDP (or discrete-action variant) instance.
    ...
    if belief_type not in _SUPPORTED_TYPES:
        raise ValueError(
            f"ContinuousLaserTagPOMDP does not support {belief_type}. "
...
def _create_particle_belief(env: "ContinuousLaserTagPOMDP", n_particles: int) -> "Belief":
    return get_initial_belief(env, n_particles)


def _create_vectorized_belief(env: "ContinuousLaserTagPOMDP", n_particles: int) -> "Belief":
