...
class SafeAntVelocityPOMDP(DiscreteActionsEnvironment):
    ...
    def cache_visualization(self, history: List[StepData], cache_path: Path) -> None:
        ...
        ax_main.set_title('Safety Ant Velocity POMDP: Trajectory & Safety Zones', fontsize=14, fontweight='bold')
        ...
        ax_speed.axhline(y=self.safe_velocity_threshold, color='orange', linestyle='--', 
                        linewidth=2, alpha=0.8, label=f'Safety Threshold ({self.safe_velocity_threshold:.1f})')
        ax_speed.axhline(y=self.safe_velocity_threshold * 1.5, color='red', linestyle='-', 
                        linewidth=2, alpha=0.8, label=f'Critical Threshold ({self.safe_velocity_threshold * 1.5:.1f})')
        
        # Initialize visual elements for main plot
        ant_scatter = ax_main.scatter([], [], s=200, c='blue', marker='o', 
                                     edgecolor='darkblue', linewidth=2, zorder=5, label='Ant')
        path_line, = ax_main.plot([], [], 'b-', alpha=0.6, linewidth=2, label='Trajectory')
        
        # Velocity vector arrow
        velocity_arrow = ax_main.annotate('', xy=(0, 0), xytext=(0, 0),
        ...
        safety_circle = plt.Circle((0, 0), 0, fill=False, edgecolor='orange', 
                                  linewidth=2, alpha=0.6, linestyle='--', visible=False)
        critical_circle = plt.Circle((0, 0), 0, fill=False, edgecolor='red', 
                                    linewidth=2, alpha=0.6, linestyle='-', visible=False)
        ...
        speed_points = ax_speed.scatter([], [], s=50, c=[], cmap='RdYlGn_r', 
                                       vmin=0, vmax=self.safe_velocity_threshold * 1.5, 
                                       edgecolor='black', linewidth=1, zorder=5)
        
        # Text displays for main plot
        step_text = ax_main.text(0.02, 0.98, '', transform=ax_main.transAxes, fontsize=12,
                                bbox=dict(boxstyle='round,pad=0.5', facecolor='lightblue', alpha=0.8),
                                verticalalignment='top', horizontalalignment='left')
        
        velocity_text = ax_main.text(0.02, 0.88, '', transform=ax_main.transAxes, fontsize=11,
                                    bbox=dict(boxstyle='round,pad=0.3', facecolor='lightyellow', alpha=0.8),
                                    verticalalignment='top', horizontalalignment='left')
        
        reward_text = ax_main.text(0.02, 0.78, '', transform=ax_main.transAxes, fontsize=11,
                                  bbox=dict(boxstyle='round,pad=0.3', facecolor='lightgreen', alpha=0.8),
                                  verticalalignment='top', horizontalalignment='left')
        
        # Safety status banner
        safety_text = ax_main.text(0.5, 0.95, '', transform=ax_main.transAxes, fontsize=14, 
        ...
        def animate(frame):
            if frame >= len(states):
                return (ant_scatter, path_line, velocity_arrow, force_arrow, 
                       speed_line, speed_points, step_text, velocity_text, 
                       reward_text, safety_text)
            ...
            safety_radius = speed / self.safe_velocity_threshold * 1.0 if self.safe_velocity_threshold > 0 else 0.1
            critical_radius = speed / (self.safe_velocity_threshold * 1.5) * 1.5 if self.safe_velocity_threshold > 0 else 0.1
            ...
            colors = ['green' if s <= self.safe_velocity_threshold 
                     else 'orange' if s <= self.safe_velocity_threshold * 1.5 
                     else 'red' for s in speeds]
            if speeds:
            ...
            velocity_text.set_text(f'Velocity: [{velocity[0]:.2f}, {velocity[1]:.2f}]\n' +
                                  f'Speed: {speed:.2f}')
