...
forward-only :class:`~POMDPPlanners.environments.nuplan_pomdp.nuplan_pomdp.NuPlanPOMDP` emits a
raw, ground-truth observation, and the model's
:meth:`~POMDPPlanners.environments.nuplan_pomdp.nuplan_generative_models.nuplan_model_pomdp.NuPlanModelPOMDP.encode_observation`
...
mid-episode, and a slot seeded empty stays empty forever. :class:`PerceivedAgentsBelief` closes
...
class PerceivedAgentsBelief(WeightedParticleBeliefReinvigoration):
    ...
    After the standard particle-filter weight update and resample, the reinvigoration step writes
    the current observation's ``agents`` block into every particle's agent slots (plus optional
    per-particle jitter), leaving the ego block to the filter and any trailing light slot
    untouched. The belief holds no perception state — perception is the planner model's, applied
    upstream by ``encode_observation`` — so a plain observation with a perceived ``agents`` block
    is all it needs.
    ...
    def __init__(
        ...
        Args:
            particles: State particles ``[ego(7) | agent slots(K*5) | optional light slot]``.
    ...
    def reinvigorate(  # type: ignore[override]
        ...
        for index in range(len(particles)):
            particles[index, EGO_STATE_WIDTH:agents_end] = self._stamped_agent_block(agent_rows)
