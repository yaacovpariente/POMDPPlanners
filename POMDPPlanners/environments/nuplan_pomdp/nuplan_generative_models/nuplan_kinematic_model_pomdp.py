...
:class:`KinematicNuPlanModelPOMDP` replaces the identity-placeholder transition of
...
This is what gives a planner a gradient toward accelerating: because acceleration now visibly
increases the along-route speed the reward rewards, POMCPOW picks controls that actually move the
car (the identity placeholder made every action look motionless).
...
class KinematicNuPlanModelPOMDP(FactoredNuPlanModelPOMDP):
    ...
    Attributes:
        ...
        wheelbase: Bicycle-model wheelbase (m) mapping steering angle to yaw rate.
        drag: Linear speed-proportional deceleration coefficient (1/s).
        collision_gap: Forward ego-frame distance (m) within which a present agent ahead is
            treated as a predicted collision by :meth:`is_terminal`.
        collision_halfwidth: Lateral ego-frame half-corridor (m) within which a present agent
            ahead is treated as a predicted collision by :meth:`is_terminal`.
        safe_distance: Lead gap (m) at/above which the reward targets the full ``desired_speed``;
            the obstacle-aware target ramps down below it.
        stop_gap: Lead gap (m) at/below which the obstacle-aware target speed is zero; ``0.0``
            keeps the flat ``desired_speed``.
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        """Initialize the kinematic nuPlan model.

        Args:
            discount_factor: Discount factor for future rewards (0 < d <= 1).
            dt: Integration step (s); match the world's ``fixed_delta_seconds``.
            action_presets: Discrete ``(acceleration, steering_angle)`` pairs.
            max_tracked_agents: Number of fixed agent slots in state/observation.
            wheelbase: Bicycle-model wheelbase (m).
            drag: Linear speed-proportional deceleration coefficient (1/s).
            collision_gap: Forward ego-frame distance (m) within which a present agent ahead
                counts as a predicted collision (terminal).
            collision_halfwidth: Lateral ego-frame half-corridor (m) within which a present agent
                ahead counts as a predicted collision (terminal).
            safe_distance: Forward ego-frame gap (m) at/above which the full ``desired_speed`` is
                targeted; below it the obstacle-aware target ramps down toward ``stop_gap``.
            stop_gap: Forward ego-frame gap (m) at/below which the obstacle-aware target speed is
                zero (the ego should be stopped). ``0.0`` keeps the flat ``desired_speed`` (no
                obstacle-aware speed shaping).
            name: Environment identifier. Defaults to the class name.
            **kwargs: Forwarded to the factored observation/reward parent (e.g. ``desired_speed``,
                ``pose_std``, ``collision_penalty``).
        """
        self.dt = dt
    ...
    def reward(self, state: Any, action: Any, next_state: Any = None) -> float:
        ...
        The bare parent reward tracks a *fixed* ``desired_speed`` and only charges the collision
        once an agent is inside the terminal box — a cliff that cannot be braked for at speed,
        while a large fixed penalty instead freezes the ego in traffic. This override follows an
        obstacle-aware desired speed: the target equals the full ``desired_speed`` when the lead
        gap is at least ``safe_distance``, ramps linearly to zero at ``stop_gap``, and is zero
        closer in. The ego is thus rewarded for driving when the road is clear and for slowing as
        an obstacle nears, without a separate penalty term that traps it at a standstill.
        ``stop_gap == 0`` keeps the flat parent behaviour.
        """
    ...
    def is_terminal(self, state: Any) -> bool:
        ...
        Because this model *does* predict ego and agent motion, it can foresee running into the
        vehicle ahead. Any present agent slot within ``collision_gap`` metres forward and
        ``collision_halfwidth`` metres laterally (ego frame) is treated as a collision, which the
        inherited reward turns into the terminal ``collision_penalty``
        (:func:`driving_quality_reward`).
        """
    ...
    def _propagate(self, state: np.ndarray, control: Tuple[float, float]) -> np.ndarray:
    ...
    def _propagate_ego(self, ego: np.ndarray, control: Tuple[float, float]) -> np.ndarray:
        acceleration, steering_angle = control
        x, y, yaw, vel_x, vel_y, lateral, heading_err = ego
    ...
    def _advance_agents(self, state: np.ndarray, vx_next: float, vy_next: float) -> None:
        ...
        state[EGO_STATE_WIDTH:agents_end] = rows.reshape(-1)
