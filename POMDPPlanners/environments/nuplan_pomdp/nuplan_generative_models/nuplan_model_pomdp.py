...
:class:`~POMDPPlanners.environments.nuplan_pomdp.nuplan_pomdp.NuPlanPOMDP` is a forward-only
...
model must satisfy: :class:`NuPlanModelPOMDP` owns only the nuPlan state/observation *schema*
...
:mod:`~POMDPPlanners.environments.nuplan_pomdp.nuplan_generative_models.nuplan_factored_model_pomdp`.
...
class NuPlanModelPOMDP(DiscreteActionsEnvironment):
    ...
    initial-distribution hooks — while this base owns the fixed nuPlan schema shared by every
    model (the ego + agent-slot state layout, the discrete action set, and the observation-dict
    hashing/equality) *and* the observation model. The observation is factored by channel: this
    base holds a ``{channel: NuPlanObservationModel}`` map (:attr:`observation_models`) and
    composes it — :meth:`sample_observation` perceives each channel of the clean observation built
    from the state, :meth:`observation_log_probability` sums the per-channel densities, and
    :meth:`encode_observation` (the single raw-observation seam) perceives each channel of the
    forward-only world's raw reading into the same perceived space, so the belief filter and
    planner search operate on one consistent (encoded) observation. Two models differ in their
    observation only by the channel models they hold; a model that scores observations for a
    belief update needs every channel to provide a density (``supports_density``). A subclass
    whose observation is not a per-channel clean transform (e.g. a learned latent decoder) may
    instead override :meth:`sample_observation`, :meth:`observation_log_probability` and
    :meth:`encode_observation` directly.

    Attributes:
        action_presets: Discrete ``(acceleration, steering_angle)`` control pairs; the discrete
            action set is the indices into this list.
        max_tracked_agents: Number of fixed agent slots in the state/observation.
        observation_models: The per-channel ``{channel: NuPlanObservationModel}`` map composed to
    ...
    def __init__(
        ...
        observation_models: Optional[Dict[str, NuPlanObservationModel]] = None,
        name: Optional[str] = None,
    ) -> None:
        """Initialize the nuPlan generative-model interface.
        ...
        Args:
            ...
            action_presets: Discrete ``(acceleration, steering_angle)`` pairs. Defaults to
                :data:`~POMDPPlanners.environments.nuplan_pomdp.nuplan_pomdp.DEFAULT_ACTION_PRESETS`.
            max_tracked_agents: Number of fixed agent slots carried in the state and observation.
                Defaults to
                :data:`~POMDPPlanners.environments.nuplan_pomdp.nuplan_pomdp.DEFAULT_MAX_TRACKED_AGENTS`.
            observation_models: The per-channel ``{channel: NuPlanObservationModel}`` map composed
    ...
    def is_equal_observation(self, observation1: Any, observation2: Any) -> bool:
        ...
        ) and np.array_equal(np.asarray(observation1["ego"]), np.asarray(observation2["ego"]))
    ...
    def _state_agent_rows(self, state: np.ndarray) -> np.ndarray:
        ...
        return state[EGO_STATE_WIDTH:agents_end].reshape(self.max_tracked_agents, AGENT_SLOT_WIDTH)
    ...
    def _clean_observation(self, state: Any) -> Dict[str, np.ndarray]:
        ...
        This is the noise-free ``{ego, agents}`` observation the perception degrades; both the
        sampler and the density route the state through it so the two agree by construction.
        """
        arr = np.asarray(state, dtype=float)
        return {
            "ego": arr[:EGO_STATE_WIDTH].copy(),
    ...
    def encode_observation(self, observation: Any) -> Any:
        ...
        perceived observation the belief filter and planner search operate in. This is the single
        raw-observation seam — every other observation method works in the perceived (encoded)
        space. A subclass without channel models (``observation_models is None``, e.g. a learned
        latent model) inherits the identity default.

        Args:
            observation: The raw ``{ego, agents}`` observation emitted by the world.
    ...
    def _composed_log_prob(
        models: Dict[str, NuPlanObservationModel], clean: Dict[str, Any], observation: Any
    ) -> float:
    ...
    def _require_observation_models(self) -> Dict[str, NuPlanObservationModel]:
        if self.observation_models is None:
            raise NotImplementedError(
                f"{type(self).__name__} has no observation models; supply a "
                "{channel: NuPlanObservationModel} map or override sample_observation."
            )
        return self.observation_models

    def _require_density_models(self) -> Dict[str, NuPlanObservationModel]:
        models = self._require_observation_models()
