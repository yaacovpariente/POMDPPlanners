...
is handled by its own :class:`NuPlanObservationModel`, and the generative model composes a
``{channel: NuPlanObservationModel}`` map. This module holds the single-channel interface;
concrete per-channel models live in the
:mod:`~POMDPPlanners.environments.nuplan_pomdp.nuplan_perception.observation_models` catalog.

Two capabilities, with different reach:

* :meth:`NuPlanObservationModel.perceive` — sample this channel's perceived value from its clean
...
* :meth:`NuPlanObservationModel.log_probability` — this channel's observation density. Optional;
...
class NuPlanObservationModel(ABC):
    ...
    Attributes:
        channel: The observation-dict key this model handles (e.g. ``"ego"`` or ``"agents"``).
