...
the selection into instances via :func:`build_observation_model`.
...
ObservationModelFactory = Callable[..., NuPlanObservationModel]
...
def register_observation_model(channel: str, name: str) -> Callable[[_FactoryT], _FactoryT]:
    ...
    Args:
        channel: The observation-dict key the model handles (e.g. ``"ego"``, ``"agents"``).
...
def build_observation_model(channel: str, name: str, **kwargs: Any) -> NuPlanObservationModel:
