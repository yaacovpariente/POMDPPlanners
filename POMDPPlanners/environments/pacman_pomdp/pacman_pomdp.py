...
class PacManPOMDP(DiscreteActionsEnvironment):
    ...
    def __init__(
        ...
        # Precompute valid positions and valid moves for each position
        self._valid_positions: Set[Tuple[int, int]] = set()
        for r in range(self.maze_size[0]):
            for c in range(self.maze_size[1]):
                pos = (r, c)
                if pos not in self.walls:
                    self._valid_positions.add(pos)

        # Precompute valid ghost moves (including stay) for each position
        self._valid_ghost_moves_cache: Dict[Tuple[int, int], List[Tuple[int, int]]] = {}
        # Precompute valid pacman/prediction moves (no stay) for each position
        self._valid_moves_cache: Dict[Tuple[int, int], List[Tuple[int, int]]] = {}
        for pos in self._valid_positions:
            ...
            valid_neighbors = [p for p in neighbors if p in self._valid_positions]
            self._valid_moves_cache[pos] = valid_neighbors
            self._valid_ghost_moves_cache[pos] = valid_neighbors + [pos]  # ghost can stay
