...
def compute_statistics_environment_policy_pair(
    ...
    Example:
        ...
        >>> # Set up environment and policy (reduced parameters for testing)
        ...
        ...     name="POMCP_Stats_Test",
        ...     n_simulations=10           # Reduced for testing
        ... )
        >>> policy.name
        'POMCP_Stats_Test'

        >>> # Run a few episodes for testing
        >>> histories = []
        >>> initial_belief = get_initial_belief(env, n_particles=10)  # Reduced for testing
        >>> logger = get_logger("stats_example", debug=False)

        >>> # Run 2 episodes for testing
        ...
        ... )
        >>> len(metrics) > 5  # Should have multiple metrics
        True

        >>> # Test that we can find specific metrics
        >>> metric_names = [m.name for m in metrics]
        >>> 'average_return' in metric_names
        True
        >>> 'return_cvar' in metric_names
        True
        >>> 'average_action_time' in metric_names
        True
    """
...
def metrics_dict_to_dataframe(
    ...
    Example:
        ...
        >>> pomcp_metrics = [
        ...     MetricValue("average_return", 10.5, 9.0, 12.0),
        ...     MetricValue("return_cvar", 8.2, 7.0, 9.4),
        ...     MetricValue("average_action_time", 0.05, 0.04, 0.06)
        ... ]
        >>> sparse_metrics = [
        ...     MetricValue("average_return", 9.8, 8.5, 11.1),
        ...     MetricValue("return_cvar", 7.9, 6.8, 9.0),
        ...     MetricValue("average_action_time", 0.02, 0.01, 0.03)
        ... ]
