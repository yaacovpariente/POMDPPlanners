...
class TestInfo:
    ...
    def __init__(self, file_path: str, function_name: str, line_number: int,
                 purpose: str, given: str, when: str, then: str, test_type: str):
    ...
    def _generate_semantic_fingerprint(self) -> str:
        ...
        stop_words = {'the', 'a', 'an', 'and', 'or', 'but', 'in', 'on', 'at', 'to', 'for', 'of', 'with', 'by', 'is', 'are', 'was', 'were', 'be', 'been', 'being', 'have', 'has', 'had', 'do', 'does', 'did', 'will', 'would', 'could', 'should', 'may', 'might', 'must', 'shall', 'can', 'that', 'this', 'these', 'those'}
...
def evaluate_same_purpose(test1: TestInfo, test2: TestInfo) -> bool:
    ...
    if any(generic in purpose1_lower for generic in generic_purposes) and \
       any(generic in purpose2_lower for generic in generic_purposes):
        return False
    ...
    specific_terms = ['config_id', 'hash', 'initialization', 'error', 'exception', 'sample', 'update']
...
def find_duplicate_tests(all_tests: List[TestInfo]) -> List[List[TestInfo]]:
    ...
    for file_path, file_tests in tests_by_file.items():
        ...
        remaining_tests = [test for test in file_tests if id(test) not in processed and test.purpose]
...
def find_pattern_duplicates(all_tests: List[TestInfo]) -> Dict[str, List[TestInfo]]:
    ...
    for file_path, file_tests in tests_by_file.items():
        ...
        for test in file_tests:
            ...
            if any(word in function_lower for word in ['init', 'initialization', 'create', 'construct']):
                file_patterns[f'initialization_in_{Path(file_path).name}'].append(test)
            
            # Error handling patterns
            if any(word in function_lower + purpose_lower for word in ['error', 'raises', 'exception', 'invalid', 'fail']):
                file_patterns[f'error_handling_in_{Path(file_path).name}'].append(test)
            
            # Configuration patterns
            if any(word in function_lower + purpose_lower for word in ['config', 'parameter', 'setting']):
                file_patterns[f'configuration_in_{Path(file_path).name}'].append(test)
            
            # Validation patterns
            if any(word in function_lower + purpose_lower for word in ['validate', 'verify', 'check', 'ensure']):
                file_patterns[f'validation_in_{Path(file_path).name}'].append(test)
            
            # State management patterns
            if any(word in function_lower + purpose_lower for word in ['state', 'update', 'modify', 'change']):
...
def generate_duplicate_report(duplicates: List[List[TestInfo]], patterns: Dict[str, List[TestInfo]], 
...
"""
    ...
    for i, group in enumerate(duplicates, 1):
        ...
        for test in group:
            ...
            report_content += f"- **{file_name}**: `{test.function_name}` (line {test.line_number})\n"
            if test.purpose:
    ...
    for pattern_name, tests in patterns.items():
        ...
        for file_name, file_tests in file_groups.items():
            ...
            for test in file_tests:
                report_content += f"- `{test.function_name}`: {test.purpose or 'No documentation'}\n"
            report_content += "\n"
...
def main():
    ...
    for root, dirs, files in os.walk(summaries_root):
        for file in files:
            if file.endswith('_summary.md') and file != 'README.md':
                ...
                print(f"  📄 Processed: {summary_file.relative_to(summaries_root)} ({len(tests)} tests)")
    ...
    if total_duplicates > 0:
        print(f"⚠️  Found {total_duplicates} potentially duplicate tests in {len(duplicates)} groups")
